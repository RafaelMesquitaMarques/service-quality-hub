...
export function KpiCard({ label, value, sub, subColor = 'text-gray-400', icon, iconBg, iconColor, trend }) {
  return (
    <div className="card p-4">
      ...
      <div className="text-2xl font-semibold text-gray-900 dark:text-gray-100 mb-0.5">{value}</div>
...
export function EmptyState({ icon = 'ti-inbox', message, action }) {
  return (
    <div className="flex flex-col items-center justify-center py-16 text-center">
      <i className={`ti ${icon} text-4xl text-gray-300 dark:text-gray-600 mb-3`} aria-hidden="true" />
...
export function Spinner({ size = 'md' }) {
  ...
  return (
    <div className={`${s} border-2 border-gray-200 border-t-blue-600 rounded-full animate-spin dark:border-gray-700 dark:border-t-blue-400`} />
  )
}
...
export function BrandTag({ brand }) {
  ...
  return (
    <span className="inline-block px-1.5 py-0.5 text-xs border border-gray-200 rounded bg-gray-50 text-gray-500 font-mono dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400">
      {brand}
    </span>
  )
}
...
export function ConfirmDialog({ open, title, message, onConfirm, onCancel, danger = false }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-[#161B22] rounded-xl shadow-xl max-w-sm w-full p-6 border border-gray-200 dark:border-gray-700">
